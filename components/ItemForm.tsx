...
export interface IItemForm {
    onFormSubmit: (obj: IMenuItem) => void, 
    onFormClose: () => void, 
    btnLabel?: string,
    iniState?: IMenuItem | null, 
};


const ItemForm = ({onFormSubmit, onFormClose, iniState = null, btnLabel}: IItemForm) => {

  const [itemEntry, setItemEntry] = useState<IMenuItem>(iniState ? iniState : {
    name: '',
    imgpath: '',
    price: 1,
    quantity: 1,
  ...
  return (
    <form className="relative flex flex-col justify-evenly items-center m-auto gap-2 w-lg min-h-[25%] border bg-theme-w p-10 rounded-xl" onSubmit={(e) => addMenuItem(e)}>
      ...
      <div className="w-full flex flex-row justify-evenly items-center mt-10">
        <ThemeButton label={btnLabel || "Add"} type="submit" />
